...
#### 深度学习：
&nbsp;&nbsp;&nbsp;&nbsp;**其实深度学习并不是一个新的方向，大约在二三十年前，是机器学习的一个分支，当时十分或火(可能是以BP网络为代表)，后来慢慢淡出了，原因主要有：**

  * 比较容易过拟合，参数较难训练，需要很多的技巧；
  * 训练速度较慢，在层数比较少(小于等于3)的情况下的效果并没有比其他方法更优。
  * BP存在的问题为：
  
	  * 梯度越来越稀疏，可能就是所谓的梯度消散问题
	  * 随机初始化会导致收敛到局部最小值
	  * 只能用有标签的样本进行训练，无法进行无监督训练。
  
  <B>所以在中间的二十年左右，神经网络被关注的很少，这段时间是svm和boost的天下。直到Hinton提出了深度网络结构。深度网络和传统网络的不同之处在于：<font color="red">为了克服神经网络训练中的问题，DL采用了神经网络很不同的训练机制。传统神经网络中，采用的是back propagation方式进行，简单来讲就是采用迭代的算法来训练整个网络，随机设定初始，计算挡墙网络的输出，然后根据挡墙输入和lable之间的差去改变前面各层的参数，直到收敛(整体上是一个梯度下降法)，而DL整体上是一个layer-wise的训练机制(先是逐层学习，后反向精调)。这样做的原因是因为，如果采用back-propagation的机制，对于一个深层网络(7层以上)，残差传播到最前面的时候已经变得太小，出现所谓的地图扩散现象。</font></B>
